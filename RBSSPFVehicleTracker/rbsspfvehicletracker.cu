...
void sampleParticle(int & pnum, VehicleState * d_particle, int & tmppnum, VehicleState * d_tmpparticle, VehicleState & estimate)
{
    ...
    estimate.a=0;estimate.v=0;estimate.k=0;
    estimate.count=0;
...
{
    VehicleState preestimate=trackerResultContainer.estimate,curestimate;
...
extern "C" bool cuda_UpdateTracker(TrackerDataContainer & trackerDataContainer, TrackerResultContainer & trackerResultContainer)
{
    ...
    EgoMotion egomotion=h_egomotion;
    if(preestimate.dx<=UNCERTAINTHRESH&&preestimate.dy<=UNCERTAINTHRESH&&preestimate.dtheta<=UNCERTAINTHRESH_ANG&&preestimate.count>=UNCERTAINTHRESH_CNT)
    {
        objectstateoffset.aoff=DEG2RAD(30);
        objectstateoffset.voff=10;
        objectstateoffset.koff=0.2;
    ...
    if(curestimate.dx<=UNCERTAINTHRESH&&curestimate.dy<=UNCERTAINTHRESH&&curestimate.dtheta<=UNCERTAINTHRESH_ANG&&curestimate.count>=UNCERTAINTHRESH_CNT)
    {
